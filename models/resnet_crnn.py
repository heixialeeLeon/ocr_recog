...
class Resnet_CRNN(nn.Module):
    def __init__(self, num_class, input_channel=3, output_channel = 512):
        super(Resnet_CRNN,self).__init__()
        self.extractor = ResNet_FeatureExtractor(input_channel,output_channel)
    ...
    def forward(self,x):
        visual_feature = self.extractor(x)
...
if __name__ == "__main__":
    crnn = Resnet_CRNN(3200,1)
    x = torch.zeros(8, 1, 32, 320)
    output = crnn(x)
    print(output.size())
